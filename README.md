...
> 💡 Для изменения параметров подключения — отредактируйте `src/main/resources/application-prod.yml` или установите
> переменную окружения `DB_PASSWORD`.
...
> ✅ **Важно**: При каждом запуске Hibernate автоматически пересоздаёт схему БД и заполняет её демо-данными из
`data.sql`.
...
Также в проект добавлена коллекция [Postman](postman_collection.json) для ручного тестирования API (содержит тесты с
проверкой статуса).
...
Настроен GitHub Actions для автоматического тестирования (CI) при каждом пуше в репозиторий. Вверху файла находится
статус сборки.
